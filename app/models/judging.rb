...
class Judging  < Salesforce
	...
	def self.queue(access_token)
		query(access_token, "select id, challenge_id__c, name, status__c, number_of_reviewers__c, end_date__c, winner_announced__c,
	...
	def self.add(access_token, challenge_id, membername)
		...
		challenge_status = query(access_token, "select id from challenge__c where status__c IN ('Planned','Created') 
			and challenge_id__c = '#{challenge_id}'")		
		# make sure all of the judging spots haven't been filled yet
		total_judges = query(access_token, "select id from challenge_reviewer__c 
			where challenge__r.challenge_id__c = '#{challenge_id}'")		
		# find out if the member is a judge already
		current_judge = query(access_token, "select id from challenge_reviewer__c where member__r.name = '#{membername}' 
			and challenge__r.challenge_id__c = '#{challenge_id}'")
		# find out if the member is a participant already
		current_participant = query(access_token, "select id from challenge_participant__c where member__r.name = '#{membername}' 
			and challenge__r.challenge_id__c = '#{challenge_id}'")
		...
		if challenge_status.count == 0
			...
			{:success => false, :message => 'Unable to add you. You are already a judge for this challenge.'}				
		elsif current_participant.count > 0
			{:success => false, :message => 'Unable to add you. You are already a participant on this challenge.'}
		elsif total_judges.count == 2
			{:success => false, :message => 'Unable to add you. There are already a maximum of two reviewers for this challenge.'}				
		else
			create(access_token, 'Challenge_Reviewer__c', 
